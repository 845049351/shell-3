...
Setting(){
	...
	sudo apt-get -y install python-software-properties && sudo add-apt-repository -y ppa:nginx/stable
   	sudo apt-get update
}
...
Edusoho(){
	...
	sudo mkdir /var/www
	cd /var/www
	sudo wget http://www.edusoho.com/edusoho-3.7.4.tar.gz
	sudo tar -zxvf edusoho-3.7.4.tar.gz && sudo chown www-data:www-data /var/www/edusoho/ -Rf
...
Mysql(){
	...
	echo 'mysql-server-5.5 mysql-server/root_password password root' | sudo debconf-set-selections   #mysql root password is 'root'
...
Php(){
	...
	sudo apt-get install -y php5 php5-cli php5-curl php5-fpm php5-intl php5-mcrypt php5-mysqlnd php5-gd
    	...
    	sudo sed -i 's/;listen.owner = www-data.*/listen.owner = www-data/g' /etc/php5/fpm/pool.d/www.conf && sudo sed -i 's/;listen.group = www-data.*/listen.group = www-data/g' /etc/php5/fpm/pool.d/www.conf
	#修改PHP.ini配置
	sudo sed -i 's/memory_limit.*/memory_limit = 1024M/g' /etc/php5/fpm/php.ini
    	...
    	sudo sed -i 's/max_execution_time.*/max_execution_time = 600/g' /etc/php5/fpm/php.ini
    	sudo sed -i 's/max_input_time.*/max_input_time = 600/g' /etc/php5/fpm/php.ini
}
